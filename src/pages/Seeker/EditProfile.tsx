...
const EditSeekerProfile = () => {
  ...
  const [form, setForm] = useState<SeekerFormState>({
    MinSalary: "",
    MaxSalary: "",
  });
      ...
      if (seekerID !== 0) {
        try {
          ...
          setForm({
            ...data,
            MinSalary:
              data.MinSalary !== null && data.MinSalary !== undefined
                ? String(data.MinSalary)
                : "",
            MaxSalary:
              data.MaxSalary !== null && data.MaxSalary !== undefined
                ? String(data.MaxSalary)
                : "",
          });
  ...
  const validateProfileImage = (file: File) => {
    ...
    if (!validFormats.includes(file.type))
      return "Profile image must be JPG or PNG";
    ...
    try {
      ...
      if (profileImage) {
        fileUploads.push(
          uploadFile(
            profileImage,
            `profileImages/${seekerID}_${profileImage.name}`
          ).then(async (url) => {
      ...
      if (cvFile) {
        fileUploads.push(
          uploadFile(cvFile, `cvFiles/${seekerID}_${cvFile.name}`).then(
            async (url) => {
      ...
      if (videoFile) {
        fileUploads.push(
          uploadFile(
            videoFile,
            `videoFiles/${seekerID}_${videoFile.name}`
          ).then(async (url) => {
      ...
      const toUpdateSkills = currentSkills.filter((s) => {
        ...
        return (
          !prev || prev.Skill !== s.Skill || prev.ExpertLevel !== s.ExpertLevel
        );
      });
      const toDeleteSkills = originalSkills.filter(
        (p) => !currentSkills.some((s) => s.id === p.id)
      );

      await Promise.all([
        ...toAddSkills.map((s) =>
          addSkill(seekerID, { Skill: s.Skill, ExpertLevel: s.ExpertLevel })
        ),
        ...toUpdateSkills.map((s) =>
          updateSkill(seekerID, s.id!, {
            Skill: s.Skill,
            ExpertLevel: s.ExpertLevel,
          })
        ),
      ...
      const careersToDelete = originalCareers.filter(
        (p) => !currentCareers.some((c) => c.id === p.id)
      );
      ...
      const educationsToDelete = originalEducations.filter(
        (p) => !currentEducations.some((e) => e.id === p.id)
      );
      ...
      if (err?.response?.status === 409) {
        ...
        setErrorMsg(
          "Invalid salary. Please check min/max salary and currency."
        );
      } else {
                ...
                return (
                  <FormControl key={field} fullWidth margin="dense">
                    ...
                    <Select
                      ...
                      label={field}
                      onChange={(e) =>
                        handleArrayChange(type, idx, field, e.target.value)
                      }
                    >
